...
chaining assignment in R.
```
<!-- Dot 1, lang: python ------------------------------------------------>
```{block, Dot = 1, part = "Instruction", lang = "python"}
**Variable**  
This Dot illustrates how to define a variable , change variable type and 
chaining assignment in Python.
```
...
**Vector**  
Vector is a basic data structure in R that contains elements of the **same type**.  
These elements can be integer , character , complex and etc.  
...
**List**  
List is one of the most common built-in data structures in python.  
it is a mutable built_in data structure in python which is used to store multiple elements.
These elements can vary in type.  
**some list methods**  
- `insert(index , element)`: puts element in place of given index.  
- `append(element)` : adds element to the end of list.  
- `pop(index)` : deletes element in given index.(and there is no given index,it will delete
...
y
#access first and second element of vector
x[c(1,2)] 
#filtering elements of x based on condition
x [x > 4] 
#modify 2nd element of x from 5 to 10
x[2] <- 10 
#modify elements less than 3 to 3
x [x < 3] <- 3 
#delete vector
y <- NULL 
y
...
**Factor**  
Factor is a data structure which is  used to represent categorical data.  
For example, a data field like gender will contain only values of male,female or other.
Each distinct value is a level.
We can consider factor as a vector of integers , that each integer has a level.  
`as.factor()` function can convert a categorical vector to a factor.  
Factors can be used in statistical modeling.
...
You can not change tuple after defining it(unlike list).
```
...
**Matrix**  
A matrix can be considered as a vector in 2 dimensions.  
Like vector , matrix can also have only one data type.  
`as.matrix()` is used to convert a `data.table` object into a matrix.  
For more information,please visit [here](https://www.datamentor.io/r-programming/matrix/).
```
...
A vector  having all elements of the same type is called an atomic vector(vector)
but a vector having elements of different types is called a `list`.  
We can also set tags(a,b,c are tags here) for list elements.Doing this,
our list will be like a `dictionary` in python.  
`as.list()` is used to convert an object to a list. These objects can be
...
**Dictionary**  
Dictionary is a data structure that is used to store a collection of `key:value`s in python.
We can access `value` in dictionary by `dict[key]`.  
In a dictionary,`value`s cab be duplicated and be of any data type.However,`key`s must be 
...
**Data Frame**  
Data Frame is a 2 dimensional data structure that is used to store tabular data.  
Data Frames can be created using `data.frame()`.  
`as.data.frame()`  is used to convert an object to a data frame.
...
**Data Frame**  
Data Frame is a 2 dimensional data structure that is used to store tabular data,and it 
is one of the most commonly used objects in `pandas` library.  
**methods**  
`df.head(n)` returns first n rows of the data frame.  
`df.tail(n)` returns last n rows from our data frame.  
`pandas.DataFrame(list)` converts the list to a data frame.  
`as.matrix(df)` converts a data frame to a numeric matrix.  
We can also convert dictionary to a data frame using 
