...
This dataset contains 891 rows and 12 columns.  
  ...
  In this Dotto , we will discuss:  
  - Introduce group by  
  - Introduce aggregation functions  
  - Multiple aggregation functions  
  - Apply a function in group by  
  - Transformation with group by  
  - Filter using group by  
  -Visualization with group by  
...
**Aggregation functions** are used  to summarize information from multiple rows into a single
row summary.  
So , once we created groups using `groupby()` function, we can create a statistical summary for
...
package to summarize multiple columns at once:  
...
from `pandas` is used to apply a function to each row or column of the data frame.  
`apply` has a `axis` argument that has default value of 0.
- if `axis=0`, then the function is applied to every row of our data frame.  
- if `axis=1`, then the function is applied to every column of our data frame.  
In this dot , we want to see that how many passengers have `pclass` equal to 3.  
    
**Introducing lambda function** :  
A lambda function is a small function that has no name when defining it,and it 
can take any number of arguments.However,it can only have one expression.  
**syntax**:  
`lambda arguments : expression`  
lambda function is usually used just once , and it can make your code shorter 
(because it is contained in just  one line of code).  
