...
Right now, we having 3 separate columns as year, month, and day in a data file in R. we want merge these three columns into just one column and make R understand that it is date. ince your year, month and day types are numerical the best function to use is the make_date function from the [lubridate](https://lubridate.tidyverse.org/index.html) package. The [tidyverse](https://www.tidyverse.org/) style solution is therefore.

There is also a simpler solution using `lubridate` and `magrittr`. This worked even we have days and months written in single (i.e. 1) and double (i.e. 01) digits. Parsing was correct as well.
...
`to_datetime()` can be used to assemble a datetime from multiple columns as well. First rename all columns, filter by values of dict. The keys (columns label) can be common abbreviations like **[‘year’, ‘month’, ‘day’, ‘minute’, ‘second’, ‘ms’, ‘us’, ‘ns’])** or plurals of the same.
...
# Solution 2
data$date2 <- paste(data$year, data$mon, data$day, sep="-") %>% 
  ymd() %>% 
  as.Date()
...
The example below is converts the `date` column to Date format and then uses `mutate_at` to create separate columns for year, month, and day.  `mutate_at` allows all three functions to be applied to the `date` column with a single line of code.
...
First, let’s create a dummy DateFrame and parse **DoB** to datetime. Then get year, month, and day.
...
# Fake data
x = data.frame(date=c('08-05-1997', '04-28-1996', '12-16-1995'))
...
df = pd.DataFrame('DoB': ['08-05-1997', '04-28-1996', '12-16-1995']})
