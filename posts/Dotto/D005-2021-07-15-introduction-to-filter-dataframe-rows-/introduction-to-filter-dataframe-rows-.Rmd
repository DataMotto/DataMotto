...
In this dot,we are using dataset `penguins_raw` ,that can be downloaded from
[here](site address).
...
`filter()` (and other `dplyr` functions) work with pipes `%>%` and  tidy data.   
In tidy data:  
...
data frame rows.  
For example , `df[df.col1 == 3800]` will select rows with `col1` = 3800.  
```
